...
function openMediaLibraryWindow(TargetInputId, TargetInputLable, MaxSelectCount) {
    ...
    wndContent = $(`<div class="k-rtl" id="addfilegrid"></div>`).kendoGrid({
        ...
        columns: [
            ...
            {
                field: "", title: "تصویر",
                template: `<img class='customer-photo  rounded'  src='/FileName/#:data.fileName#' onclick=" img_modal('/FileName/#:data.fileName#','#:data.title#')" style='cursor: pointer;max-width:40px'> `
            },
            {
                field: "", title: "عنوان",
                template: '#:data.title#'
...
function img_modal(imgSource, Title) {
    ...
    var wndContent = `<img src="${imgSource}" style="height:80vh"></img>`
